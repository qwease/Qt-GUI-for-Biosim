...
// Function to convert CImg to QImage
QImage cimgToQImage(const cimg_library::CImg<uint8_t> &cimg) {
    QImage image(cimg.width(), cimg.height(), QImage::Format_RGB32);
    cimg_forXY(cimg, x, y) {
        uint8_t r = cimg(x, y, 0, 0);
        uint8_t g = cimg(x, y, 0, 1);
        uint8_t b = cimg(x, y, 0, 2);
...
void setLastFrame(QImage frame){
    lastFrame=frame;
}
...
void saveOneFrameImmed(const ImageFrameData &data)
{
    ...
    color[0] = color[1] = color[2] = 0x88;//barrier color (alpha value)
    for (Coord loc : data.barrierLocs) {
            image.draw_rectangle(
                loc.x       * p.displayScale - (p.displayScale / 2),
                ((p.sizeY - loc.y) - 1)   * p.displayScale - (p.displayScale / 2),
                (loc.x + 1) * p.displayScale,
                ((p.sizeY - (loc.y - 0))) * p.displayScale,
                color,  // rgb
                1.0);  // alpha
    }
    ...
    setLastFrame(cimgToQImage(image));
...
// right version for endOfSimStep
bool ImageWriter::saveVideoFrameSync(unsigned simStep, unsigned generation)
{
    ...
    saveOneFrameImmed(data); // convert from data to "img"

    // copy image list
    // BS::displayableImageList = imageList;
    return true;
}
