...
{
    // creat net.txt
    QString netDataFile = "./tools/net.txt";
    QFile netData(netDataFile);
    netData.resize(0);
    if (!netData.open(QIODevice::ReadWrite | QIODevice::Text)) {
        return;
    }

    // open up textstream
    QTextStream stream(&netData);

    // redirect the following into the file
    for (auto & conn : nnet.connections) {
        if (conn.sourceType == SENSOR) {
            stream << QString::fromStdString(sensorShortName((Sensor)(conn.sourceNum)));
        } else {
            stream << "N" << QString::number(conn.sourceNum);
        }

        stream << " ";

        if (conn.sinkType == ACTION) {
            stream << QString::fromStdString(actionShortName((Action)(conn.sinkNum)));
        } else {
            stream << "N" << QString::number(conn.sinkNum);
        }

        stream << " " << QString::number(conn.weight) << "\n";
    }
    ...
    //run python file
        ...
        // UNIX-LIKE
        returnValue = system("python3 ./tools/graph-nnet.py");
    #elif _WIN32
        //WINDOWS
        returnValue = system("start python3 ./tools/graph-nnet.py");
    #endif
    if (returnValue < 0)
...
void displaySignalUse()
{
    ...
    for (int16_t x = 0; x < p.sizeX; ++x) {
        for (int16_t y = 0; y < p.sizeY; ++y) {
            unsigned magnitude = pheromoneSignals.getMagnitude(0, { x, y });
            if (magnitude != 0) {
                ++count;
                sum += magnitude;
            }
