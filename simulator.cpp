...
void runSimulation(char const * filename){
    qDebug()<<filename;
    runMode=RunMode::RUN;
    QFuture<void> future = QtConcurrent::run(BS::simulator,2,filename);
}
...
void resetSimulation() {
    #pragma omp flush(reset)
    reset = true;
}

void simulator(QPromise<void> &promise, int argc, const char* filename)
{
    ...
    #pragma omp parallel num_threads(p.numThreads) default(shared)
    {
        ...
        while (runMode == RunMode::RUN && generation < p.maxGenerations) { // generation loop
            ...
            while (!reset) { // Simulation loop
                if (shouldExit) {
                    break;
                }

                std::unique_lock<std::mutex> lock(pauseMutex);
                pauseCondVar.wait(lock, [&]() { return !paused; });
                lock.unlock(); // Unlock the mutex before running the simulation
            ...
            #pragma omp flush(reset,paused, shouldExit)
            if (reset) {
                // Reinitialize the simulation
                #pragma omp single
                {
                    endOfGeneration(generation);
                    qDebug() << "endOfGeneration";
                    generation = 0;
                    paramManager.updateFromConfigFile(0);
                    qDebug() << "updateFromConfigFile";
                    initializeGeneration0(); // starting population
                    qDebug() << "initializeGeneration0";
                }
